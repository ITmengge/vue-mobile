...
const service = axios.create({
    baseURL: 'http://classroom2.gz2vip.91tunnel.com', // api 的 base_url
...
service.interceptors.response.use(response => {
        if (response.data.code == 208) {
            //debugger
            // 替换# 后台获取不到#后面的参数
            // 将路径中的#转为simon，注意不要跟内网穿透地址的路径一样了（如classroom），不然就会替换掉
            // 出现returnUrl:http://#.free.idcfengye.com/#/course/1
            let url = window.location.href.replace('#', 'simon')
            window.location = 'http://classroom2.gz2vip.91tunnel.com/api/user/wechat/authorize?returnUrl=' + url
        } else {
